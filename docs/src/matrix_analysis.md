...
symmetric polynomials, and multiplicative and additive compound matrices. Its
contracts prefer mathematical shape conventions, exact arithmetic where
possible, and explicit sparse behavior over reproducing incidental behavior of
the pinned MATLAB implementation.
...
sorting could otherwise densify them implicitly.
...
native function for the mathematically defined `1×1` result. Full signatures
are collected in the [API reference](api/index.md).
