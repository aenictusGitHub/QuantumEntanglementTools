...
Julia API, provenance, tests, and documentation were compared in the
agent-assisted inventory sweep and a deliberate source disposition was
recorded. It is a classification milestone. It is not an independent
per-function derivation of every mathematical contract, implementation
verification, MATLAB parity, or evidence that a human maintainer completed the
non-delegable release review.
...
dated source-review counts remain as the baseline evidence actually observed
on 2026-07-29. The new result is local static repository evidence, not
...
| Deferred | 19 | Implementable work was outside the first milestone's verified scope; two were already deferred and 17 were classified in this sweep |
...
Blocked does not mean impossible or outside the terminal project goal. It means
that marking the row implemented before the named prerequisite exists would
violate the package's solver, certificate, RNG, or validation rules.
...
The two rows that were deferred before this source-review baseline,
`RandomSuperoperator` and `RandomPPTState`, have since been implemented with
explicit RNG, bounded work, and structured verification contracts. Their
current dispositions are recorded in the generated inventory.
...
`pure_to_mixed`, `jacobi_poly`, and `sporth`.
...
`FP2FC`, `has_band_k_ordering`, `normalize_cols`, `one_factorization`,
`poly_rand_input`, `sk_iterate`, `spnull`, `symindfind`, and
`vec_partitions`.
