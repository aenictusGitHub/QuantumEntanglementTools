...
output_path="${repository_root}/test/oracle/generated/twirl_oracle.json"
engine="auto"
...
actual_source_sha="$(shasum -a 256 "${qetlab_path}/Twirl.m" | awk '{print $1}')"
expected_source_sha="a2ed4de3c937cb3e451d0b8e84508cdbe4b3bc23013558015d0e2b972853cd33"
if [[ "$actual_source_sha" != "$expected_source_sha" ]]; then
    printf 'Refusing Twirl.m with unexpected SHA-256: expected %s, found %s.\n' \
...
if [[ "$selected_engine" == "matlab" ]]; then
    ...
    "$matlab_binary" -batch \
        "addpath('${escaped_directory}'); run_twirl_oracle"
else
