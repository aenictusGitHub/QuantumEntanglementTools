...
output_path="${repository_root}/test/oracle/generated/sk_norms_oracle.json"
engine="auto"
...
if [[ ! -f "${qetlab_path}/SkOperatorNorm.m" ]]; then
...
if [[ "$selected_engine" == "matlab" ]]; then
    ...
    "$matlab_binary" -batch \
        "addpath('${escaped_directory}'); run_sk_norms_oracle"
else
