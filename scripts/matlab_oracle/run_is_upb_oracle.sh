...
output_path="${repository_root}/test/oracle/generated/is_upb_oracle.json"
engine="auto"
...
if [[ ! -f "${qetlab_path}/IsUPB.m" ]] || \
        [[ ! -f "${qetlab_path}/helpers/vec_partitions.m" ]]; then
    printf 'Pinned IsUPB sources not found at %s.\n' "$qetlab_path" >&2
    exit 2
fi
...
if [[ "$selected_engine" == "matlab" ]]; then
    ...
    "$matlab_binary" -batch \
        "addpath('${escaped_directory}'); run_is_upb_oracle"
else
