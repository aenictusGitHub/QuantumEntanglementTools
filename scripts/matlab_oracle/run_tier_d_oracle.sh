...
output_path="${repository_root}/test/oracle/generated/tier_d_oracle.json"
engine="auto"
...
if [[ ! -f "${qetlab_path}/TraceNorm.m" ]]; then
...
if [[ "$selected_engine" == "matlab" ]]; then
    ...
    "$matlab_binary" -batch \
        "addpath('${escaped_directory}'); run_tier_d_oracle"
else
