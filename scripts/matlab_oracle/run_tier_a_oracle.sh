...
output_path="${repository_root}/test/oracle/generated/tier_a_oracle.json"
engine="auto"
...
usage() {
    printf '%s\n' \
        ...
        "" \
        "MATLAB is preferred in auto mode. Octave is a supplemental, function-" \
        "specific check and is never labeled generally MATLAB-equivalent."
}
...
if [[ ! -f "${qetlab_path}/PartialTrace.m" ]]; then
...
if [[ "$selected_engine" == "matlab" ]]; then
    ...
    "$matlab_binary" -batch \
        "addpath('${escaped_directory}'); run_tier_a_oracle"
else
    if ! command -v "$octave_binary" >/dev/null 2>&1; then
        printf 'Octave executable not found: %s\n' "$octave_binary" >&2
        exit 2
    fi
    "$octave_binary" --quiet --no-gui --path "$script_directory" \
        --eval "run_tier_a_oracle"
fi
