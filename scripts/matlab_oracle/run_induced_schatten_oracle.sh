...
output_path="${repository_root}/test/oracle/generated/induced_schatten_oracle.json"
engine="auto"
...
if [[ ! -f "${qetlab_path}/InducedSchattenNorm.m" ]]; then
...
if [[ "$selected_engine" == "matlab" ]]; then
    ...
    "$matlab_binary" -batch \
        "addpath('${escaped_directory}'); run_induced_schatten_oracle"
else
    ...
    "$octave_binary" --quiet --no-gui --path "$script_directory" \
        --eval "run_induced_schatten_oracle"
fi
