...
output_path="${repository_root}/test/oracle/generated/nonlocal_games_oracle.json"
engine="auto"
...
for required in XORGameValue.m BellInequalityMax.m; do
    if [[ ! -f "${qetlab_path}/${required}" ]]; then
        printf 'QETLAB checkout at %s is missing %s.\n' "$qetlab_path" "$required" >&2
        exit 2
    fi
done
if [[ ! -f "${qetlab_path}/helpers/bcs_to_nonlocal.m" ]]; then
    printf 'QETLAB checkout at %s is missing helpers/bcs_to_nonlocal.m.\n' \
        "$qetlab_path" >&2
    exit 2
fi
...
dirty_paths="$(git -C "$qetlab_path" status --porcelain --untracked-files=all)"
if [[ -n "$dirty_paths" ]]; then
    printf 'Refusing dirty QETLAB checkout at %s; oracle evidence requires the exact clean tree.\n' \
        "$qetlab_path" >&2
    printf '%s\n' "$dirty_paths" >&2
    exit 2
fi
...
if [[ "$selected_engine" == "matlab" ]]; then
    ...
    "$matlab_binary" -batch \
        "addpath('${escaped_directory}'); run_nonlocal_games_oracle"
else
    ...
    "$octave_binary" --quiet --no-gui --path "$script_directory" \
        --eval "run_nonlocal_games_oracle"
fi
