...
output_path="${repository_root}/test/oracle/generated/entangling_gate_oracle.json"
engine="auto"
...
if [[ ! -f "${qetlab_path}/IsEntanglingGate.m" ]]; then
...
if [[ "$selected_engine" == "matlab" ]]; then
    ...
    "$matlab_binary" -batch \
        "addpath('${escaped_directory}'); run_entangling_gate_oracle"
else
    ...
    "$octave_binary" --quiet --no-gui --path "$script_directory" \
        --eval "run_entangling_gate_oracle"
fi
