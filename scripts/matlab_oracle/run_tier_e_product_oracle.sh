...
output_path="${repository_root}/test/oracle/generated/tier_e_product_oracle.json"
engine="auto"
...
if [[ ! -f "${qetlab_path}/OperatorSchmidtDecomposition.m" ]]; then
...
if [[ "$selected_engine" == "matlab" ]]; then
    ...
    "$matlab_binary" -batch \
        "addpath('${escaped_directory}'); run_tier_e_product_oracle"
else
    ...
    "$octave_binary" --quiet --no-gui --path "$script_directory" \
        --eval "run_tier_e_product_oracle"
fi
