...
output_path="${repository_root}/test/oracle/generated/channel_optimization_oracle.json"
engine="auto"
...
if [[ "$selected_engine" == "matlab" ]]; then
    ...
    "$matlab_binary" -batch \
        "addpath('${escaped_directory}'); run_channel_optimization_oracle"
else
    ...
    "$octave_binary" --quiet --no-gui --path "$script_directory" \
        --eval "run_channel_optimization_oracle"
fi
