...
output_path="${repository_root}/test/oracle/generated/coherence_optimization_oracle.json"
engine="auto"
...
if [[ ! -f "${qetlab_path}/IskIncoherent.m" ]]; then
...
if [[ "$selected_engine" == "matlab" ]]; then
    ...
    "$matlab_binary" -batch \
        "addpath('${escaped_directory}'); run_coherence_optimization_oracle"
else
    ...
    "$octave_binary" --quiet --no-gui --path "$script_directory" \
        --eval "run_coherence_optimization_oracle"
fi
