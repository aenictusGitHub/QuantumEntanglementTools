...
output_path="${repository_root}/test/oracle/generated/minimum_upb_size_oracle.json"
engine="auto"
...
if [[ ! -f "${qetlab_path}/MinUPBSize.m" ]]; then
...
if [[ "$selected_engine" == "matlab" ]]; then
    ...
    "$matlab_binary" -batch \
        "addpath('${escaped_directory}'); run_minimum_upb_size_oracle"
else
    ...
    "$octave_binary" --quiet --no-gui --path "$script_directory" \
        --eval "run_minimum_upb_size_oracle"
fi
