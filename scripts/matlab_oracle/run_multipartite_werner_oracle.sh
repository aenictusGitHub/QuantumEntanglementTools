...
output_path="${repository_root}/test/oracle/generated/multipartite_werner_oracle.json"
engine="auto"
...
if [[ ! -f "${qetlab_path}/WernerState.m" ]]; then
...
if [[ "$selected_engine" == "matlab" ]]; then
    ...
    "$matlab_binary" -batch \
        "addpath('${escaped_directory}'); run_multipartite_werner_oracle"
else
    ...
    "$octave_binary" --quiet --no-gui --path "$script_directory" \
        --eval "run_multipartite_werner_oracle"
fi
