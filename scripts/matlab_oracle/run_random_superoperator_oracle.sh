...
output_path="${repository_root}/test/oracle/generated/random_superoperator_oracle.json"
engine="auto"
expected_commit="d8589610f00cff106537268dee2e2a1153f3a601"
expected_source_sha="ac2862092569e0878c0f5dac68f4cd58670992ae44adea26d2d7dfb36c07d96f"
...
if [[ ! -f "${qetlab_path}/RandomSuperoperator.m" ]]; then
...
actual_source_sha="$(shasum -a 256 "${qetlab_path}/RandomSuperoperator.m" | awk '{print $1}')"
if [[ "$actual_source_sha" != "$expected_source_sha" ]]; then
    printf 'Refusing RandomSuperoperator.m with unexpected SHA-256: expected %s, found %s.\n' \
...
if [[ "$selected_engine" == "matlab" ]]; then
    ...
    "$matlab_binary" -batch \
        "addpath('${escaped_directory}'); run_random_superoperator_oracle"
else
    ...
    "$octave_binary" --quiet --no-gui --path "$script_directory" \
        --eval "run_random_superoperator_oracle"
fi
