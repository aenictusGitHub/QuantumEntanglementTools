...
output_path="${repository_root}/test/oracle/generated/entangled_subspace_oracle.json"
engine="auto"
...
if [[ ! -f "${qetlab_path}/EntangledSubspace.m" ]]; then
...
if [[ "$selected_engine" == "matlab" ]]; then
    ...
    "$matlab_binary" -batch \
        "addpath('${escaped_directory}'); run_entangled_subspace_oracle"
else
    ...
    "$octave_binary" --quiet --no-gui --path "$script_directory" \
        --eval "run_entangled_subspace_oracle"
fi
