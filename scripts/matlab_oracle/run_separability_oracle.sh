...
output_path="${repository_root}/test/oracle/generated/separability_oracle.json"
engine="auto"
...
for required_file in IsSeparable.m LocalDistinguishability.m UPBSepDistinguishable.m; do
    if [[ ! -f "${qetlab_path}/${required_file}" ]]; then
        printf 'QETLAB checkout at %s does not contain %s.\n' \
            "$qetlab_path" "$required_file" >&2
        exit 2
    fi
done
...
if [[ "$selected_engine" == "matlab" ]]; then
    ...
    "$matlab_binary" -batch \
        "addpath('${escaped_directory}'); run_separability_oracle"
else
