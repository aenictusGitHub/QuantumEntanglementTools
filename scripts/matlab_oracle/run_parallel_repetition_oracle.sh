...
output_path="${repository_root}/test/oracle/generated/parallel_repetition_oracle.json"
engine="auto"
...
if [[ ! -f "${qetlab_path}/ParallelRepetition.m" ]]; then
...
if [[ "$selected_engine" == "matlab" ]]; then
    ...
    "$matlab_binary" -batch \
        "addpath('${escaped_directory}'); run_parallel_repetition_oracle"
else
    ...
    "$octave_binary" --quiet --no-gui --path "$script_directory" \
        --eval "run_parallel_repetition_oracle"
fi
