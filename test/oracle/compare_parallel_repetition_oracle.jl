...
const FIXTURE_NAMES = Set([
    "parallel_repetition_one_copy", "parallel_repetition_two_copies"
])
...
function fixture_array(fixture)
    ...
    length(dimensions) == 4 ||
        error("fixture $(fixture.name) does not record four dimensions")
...
committed_fixture = joinpath(
    @__DIR__, "fixtures", "parallel_repetition_octave_11_3_qetlab_d858961.json"
)
fixture_path = if isempty(ARGS)
    generated = joinpath(@__DIR__, "generated", "parallel_repetition_oracle.json")
    isfile(generated) ? generated : committed_fixture
else
    abspath(ARGS[1])
end
isfile(fixture_path) || error(
    "ParallelRepetition oracle fixture not found at $fixture_path; " *
    "run scripts/matlab_oracle/run_parallel_repetition_oracle.sh first",
)
...
String(metadata.tier) == "WP2-parallel-repetition" ||
...
occursin("most-significant", String(metadata.copy_order)) ||
    error("fixture does not record packed-copy order")
...
game = reshape(Float64.(1:24), 2, 3, 2, 2)

@testset "QETLAB ParallelRepetition differential fixture" begin
    for fixture in payload.fixtures
        name = String(fixture.name)
        repetitions = name == "parallel_repetition_one_copy" ? 1 : 2
        expected = fixture_array(fixture)
        native = QET.parallel_repetition(game, repetitions)
        compatibility = Compat.ParallelRepetition(game, repetitions)
        @test String(fixture.comparison) == "exact"
        @test size(native) == size(expected)
        @test native == expected
        @test compatibility == expected
    end
end

println(
    "ParallelRepetition oracle comparison passed for ",
